            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th class="text-left py-4 px-6 font-semibold text-gray-700 text-sm">ID</th>
                    ...
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors duration-150">
                        <td class="py-4 px-6 text-gray-800 font-medium">{{ $report->report_id }}</td>
                        <td class="py-4 px-6 text-gray-800">{{ $report->report_name }}</td>
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('reports.edit', $report->report_id) }}" class="p-2 text-gray-400 hover:text-orange-500 hover:bg-orange-50 rounded-lg transition-all duration-200" title="Edit">
                                ...
                                <form action="{{ route('reports.destroy', $report->report_id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all duration-200" title="Delete">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <a href="{{ route('reports.show', $report->report_id) }}" class="p-2 text-gray-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all duration-200" title="View">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
